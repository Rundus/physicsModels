# --- imports ---
from ionosphere.simToggles_iono import GenToggles, runFullSimulation
from spaceToolsLib.variables import lightSpeed,u0,m_e,ep0,cm_to_m,IonMasses,q0, m_to_km,Re
...
##################
plotting = False
xNorm = Re # use m_to_km otherwise
...
def generatePlasmaEnvironment(outputData, **kwargs):
    ...
    def Intertial_alfvenSpeedProfile(altRange, **kwargs):
        ...
        if plotBool:
            ...
            ax.axhline(y=Vth_low, color='black',linewidth=Plot_LineWidth)
            ax.text(x=Re/xNorm,y=Vth_low*2,s='$V_{th_{e}}$ (1 eV)', color='black',fontsize=Text_Fontsize)

            Vth_high = sqrt(8 * q0 * 50 / (9.11E-31))/(m_to_km*10000)
            ax.axhline(y=Vth_high, color='black',linewidth=Plot_LineWidth)
            ax.text(x=Re/xNorm, y=Vth_high * 1.3, s='$V_{th_{e}}$ (50 eV)', color='black',fontsize=Text_Fontsize)
