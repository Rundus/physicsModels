# --- imports ---
from ionosphere.simToggles_iono import GenToggles,BgeoToggles,runFullSimulation
...
def generateGeomagneticField(outputData, **kwargs):
    ...
    altRange = kwargs.get('Alt', [])
    constantBval = kwargs.get('constantBval',None)

    def geomagneticFieldProfile(altRange, **kwargs):
        plotBool = kwargs.get('showPlot', False)

        if constantBval != None:
            # MAKE IT A CONSTANT
            Bgeo = array([constantBval for alt in altRange])
            Bgrad = array([0 for alt in altRange])
        else:

            geomagAlts = [((alt + Re) / Re) for alt in altRange]
        ...
        if plotBool:
            ...
            ax.plot(altRange/Re, Bgeo/(1E-9),linewidth=Plot_LineWidth, color='black',label=r'$\vec{B}_{geo}$')
            ...
            ax.set_yscale('log')
            ax.axvline(x=400000/Re,label='Observation Height',color='red',linewidth=Plot_LineWidth)
            ax.plot(altRange / Re, Bgrad / (1E-9), color='black', linewidth=Plot_LineWidth, linestyle='--', label=r'$\nabla B$', alpha=1)
            ax.set_ylim(1E3,1E5)
            ax.legend(fontsize=Legend_fontSize)

            axBGrad = ax.twinx()
            axBGrad.plot(altRange / Re, Bgrad/(1E-9), color='black',linewidth=Plot_LineWidth, linestyle='--',label=r'$\nabla B$')
            axBGrad.set_ylabel(r'$\nabla B$ [nT/m]', fontsize=Label_FontSize)
            axBGrad.axvline(x=400000 / Re, label='Observation Height', color='red',linewidth=Plot_LineWidth)
...
generateGeomagneticField(outputData=outputData_Bool,plotting=plot_BField)