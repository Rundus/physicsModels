...
from src.ionosphere_modelling.conductivity.conductivity_toggles import *
from src.ionosphere_modelling.sim_toggles import *
...
data_dict_Bgeo = loadDictFromFile(glob(rf'{SimToggles.sim_root_path}\geomagneticField\geomagneticField.cdf')[0])

# get the ionospheric neutral data dict
data_dict_neutral = loadDictFromFile(glob(rf'{SimToggles.sim_root_path}\neutral_environment\neutral_environment.cdf')[0])

# get the ionospheric plasma data dict
data_dict_plasma = loadDictFromFile(glob(rf'{SimToggles.sim_root_path}\plasma_environment\plasma_environment.cdf')[0])
...
# get the ionospheric conductivity
...
def makePlotOfModel(altRange, data_dict):
    import matplotlib.pyplot as plt
    fig, ax = plt.subplots(nrows=2,ncols=3)
    sizeVal = 20
    fig.set_size_inches(sizeVal, sizeVal*2/3)

    # CHOOSE the L-Shell to gather the data from
    data_idx = 2000
    ...
    for key in neutralsToggles.wNeutrals:
        ax[0, 0].plot(data_dict_neutral[f'{key}'][0][data_idx], altRange / xNorm, linewidth=Plot_LineWidth,label=key)  # neutrals

    for key in plasmaToggles.wIons:
        ax[0, 1].plot(data_dict_plasma[f'n_{key}'][0][data_idx], altRange / xNorm, linewidth=Plot_LineWidth,label=key) # ions
        ax[1, 1].plot(data_dict[f'sigma_P_{key}'][0][data_idx]*conductivityScaleFactor, altRange / xNorm, linewidth=Plot_LineWidth,label=key) # ion pedersen conductivites
        ax[1, 2].plot(data_dict[f'sigma_H_{key}'][0][data_idx]*conductivityScaleFactor, altRange / xNorm, linewidth=Plot_LineWidth,label=key)  # ion Hall conductivites


    ax[1,1].set_xlabel('Pedersen Conductivity [S/10km]', fontsize=Label_FontSize)

    # neutrals - fine adjustments
    ax[0, 0].set_ylabel('Altitude [km]', fontsize=Label_FontSize)
    ax[0, 0].set_xlim(1E12, 1E21)
    ax[0, 0].set_xscale('log')
    ax[0, 0].set_xlabel('Neutral Number density [m$^{-3}$]', fontsize=Label_FontSize)

    # ions - fine adjustments
    ax[0, 1].set_xlim(1E9, 1E12)
    ax[0, 1].set_xscale('log')
    ax[0, 1].set_xlabel('Ion Number density [m$^{-3}$]', fontsize=Label_FontSize)

    # temperature
    ax[0,2].set_xlim(0, 1800)
    ax[0, 2].set_xlabel('Temperature [K]', fontsize=Label_FontSize)
    ax[0, 2].plot(data_dict_plasma[f'Ti'][0][data_idx], altRange / xNorm, linewidth=Plot_LineWidth,label='$T_{i}$')  # temperature
    ax[0, 2].plot(data_dict_plasma[f'Te'][0][data_idx], altRange / xNorm, linewidth=Plot_LineWidth,label='$T_{e}$')  # temperature
    ax[0, 2].plot(data_dict_neutral[f'Tn'][0][data_idx], altRange / xNorm, linewidth=Plot_LineWidth,label='$T_{n}$')  # temperature

    # Total Pedersen/Hall Conductivity
    ax[1, 0].plot(data_dict['sigma_P_total'][0][data_idx]*conductivityScaleFactor, altRange / xNorm, linewidth=Plot_LineWidth,label='$\sigma_{P}$')
    ax[1, 0].plot(data_dict['sigma_H_total'][0][data_idx]*conductivityScaleFactor, altRange / xNorm, linewidth=Plot_LineWidth,label='$\sigma_{H}$')
    ax[1, 0].set_xlabel('Conductivity [S/10km]', fontsize=Label_FontSize)
    ax[1, 0].set_ylabel('Altitude [km]', fontsize=Label_FontSize)

    # Hall conductivity
    ax[1, 2].set_xlabel('Hall Conductivity [S/10km]', fontsize=Label_FontSize)
    ...
    plt.tight_layout()
    plt.savefig(rf'{SimToggles.sim_root_path}\conductivity\test_scripts\MODEL_profile.png', dpi=dpi)

makePlotOfModel(data_dict_spatial['simAlt'][0],data_dict_conductivity)
...
#
#
# def generateIonosphericConductivity(GenToggles, **kwargs):
#     data_dict = {}
#
#     # --- Electron mobility ---
...
#             plt.tight_layout()
#             plt.savefig(rf'{GenToggles.simFolderPath}\conductivity\test_scripts\MODEL_Electron_CollisionFreq.png', dpi=dpi)
#
...
#             plt.legend(fontsize=Legend_fontSize)
#             plt.tight_layout()
#             plt.savefig(rf'{GenToggles.simFolderPath}\conductivity\test_scripts\MODEL_Ion_CollisionFreq.png', dpi=dpi)
#
#         # update the data_dict
#         return data_dict
#
...
#             plt.legend(fontsize=Legend_fontSize)
#             plt.tight_layout()
#             plt.savefig(rf'{GenToggles.simFolderPath}\conductivity\test_scripts\MODEL_mobility.png', dpi=dpi)
#
#         # update the data_dict
#         return data_dict
#
...
#             plt.legend(fontsize=Legend_fontSize)
#             plt.tight_layout()
#             plt.savefig(rf'{GenToggles.simFolderPath}\conductivity\test_scripts\MODEL_conductivity.png', dpi=dpi)
#
#         return data_dict
#
#     if kwargs.get('showPlot', False):
...
#
#     outputPath = rf'{GenToggles.simFolderPath}\conductivity\test_scripts\test_scripts_conductivity.cdf'
...
# # --- EXECUTE ---
# #################
# stl.prgMsg('Verifying Conductivity Models')
# generateIonosphericConductivity(GenToggles, showPlot=True)
# stl.Done(start_time)