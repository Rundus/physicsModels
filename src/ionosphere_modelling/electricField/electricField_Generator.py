def generate_electricField():
    ...
    for j in range(len(altRange)):
        ...
        grid_E_N[:, j] = -1 * np.gradient(data_dict_potential['potential'][0][:, j], data_dict_spatial['grid_dN'][0][:, j])

    # Vertical
    for i in range(len(LShellRange)):
        # Calculate E = -Grad(Phi)
        grid_E_p[i, :] = -1 * np.gradient(data_dict_potential['potential'][0][i, :], data_dict_spatial['grid_dp'][0][i, :])
